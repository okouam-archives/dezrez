...
class Portals
  ...
  def push(portal)
    buy_properties = Subscriber.new.pull(@config["image_directory"], 6)
    rent_properties = Subscriber.new.pull(@config["image_directory"], 1)
    properties = buy_properties + rent_properties
